---
subject: '[Algorithm] LeetCode : Group Anagrams'
date: '2021-06-01'
category: ['Algorithm']
...
<Typography variant="h3" backgroundColor="gray">
  <span>알고리즘 기초 다지기 프로젝트 (feat. 코드없는 프로그래밍) \[2021년 06월 01일\]</span>
</Typography>
...
<Typography variant="h4">
  <span>Leetcode - Group Anagrams</span>
</Typography>
...
-   코드 (1) : 통과 성공
  ...
  const calcAlphaCnt = (arrStr, searchChar) =>
    arrStr.reduce(
      (cnt, currChar) => (currChar === searchChar && cnt++, cnt),
      0,
    );
  const isDuplicateKey = (object, key) =>

...
배열은 `Symbol.iterator` 을 <u>소유</u>한다. 따라서 배열은 이터러블 프로토콜을 <Typography variant="span" color="blue">준수한 이터러블</Typography>이다.
...
일반 객체는 `Symbol.iterator`을 <u>소유하지 않는다</u>. 따라서 일반객체는 이터러블 프로토콜을 <Typography variant="span" color="red">준수한 이터러블이 아니다.</Typography>
...
이터러블 프로토콜을 <Typography variant="span" color="blue">준수한 이터러블</Typography>은 `Symbol.iterator` 메소드를 소유한다.  
...
<Typography id="iterlist" variant="span" color="brown" isBold>ES6에서 제공하는 빌트인 이터러블 목록</Typography>
